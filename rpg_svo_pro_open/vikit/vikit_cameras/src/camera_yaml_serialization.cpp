...
bool convert<std::shared_ptr<vk::cameras::CameraGeometryBase> >::decode(
  ...
  try {
    ...
    std::string distortion_type; // type
    Eigen::VectorXd distortion_parameters; // params
    ...
    if(YAML::safeGet(distortion_config, "type", &distortion_type)/* radial-tangential */ &&
       YAML::safeGet(distortion_config, "parameters", &distortion_parameters) &&
       YAML::safeGet(node, "type", &camera_type)/* pinhole */ &&
       YAML::safeGet(node, "image_width", &image_width)/* 1280 */ &&
       YAML::safeGet(node, "image_height", &image_height)/* 1024 */ &&
       YAML::safeGet(node, "intrinsics", &intrinsics))
    {
      ...
      else if(camera_type == "pinhole" && distortion_type == "radial-tangential") // HERE
      {
