    ...
    std::vector<PointPtr>& trash_points)
{
  ...
  for(const FramePtr& ref_frame : visible_kfs)
  {
    ...
    for(size_t i = 0; i < ref_frame->num_features_; ++i)
    {
      ...
      if(ref_frame->landmark_vec_[i] == nullptr  // 种子点这里就会跳过
  ...
  reprojector_utils::matchCandidates(
        ...
        options_.affine_est_offset, options_.affine_est_gain,
        candidates_/* 其中的点位置被优化了 */, *grid_, lm_stats, options_.seed_sigma2_thresh);
  ...
  for(const FramePtr& ref_frame : visible_kfs)
  {
    for(size_t i = 0; i < ref_frame->num_features_; ++i)
    {
      if(isConvergedCornerEdgeletSeed(ref_frame->type_vec_[i])) // 若是收敛的角点或者边缘点seed
      {
        Candidate candidate;
        if(reprojector_utils::getCandidate(cur_frame, ref_frame, i, candidate))
          candidates_.push_back(candidate);
    ...
    const double seed_sigma2_thresh)
{
  ...
  for(Reprojector::Candidate& candidate : candidates)
  {
    ...
    ++stats.n_trials;
    FeatureWrapper feature_wrapper = frame->getEmptyFeatureWrapper(); // 这是一个引用
    if(matchCandidate(frame, candidate, matcher, feature_wrapper,
                      seed_sigma2_thresh))
    {
      ++stats.n_matches;
      ++frame->num_features_; // 使得 feature_wrapper 被不断更新
    ...
    const double seed_sigma2_thresh)
{
  ...
  int track_id = -1;
  if(c.ref_frame->landmark_vec_.at(c.ref_index) == nullptr) // Seed 情况（收敛或者没收敛都走这个分支），没生成 3D 点
  {
    FeatureWrapper ref_ftr = c.ref_frame->getFeatureWrapper(c.ref_index); // 把 Frame 中有关 Feature 的内容封装在一起
    if(isConvergedCornerEdgeletSeed(c.type) || isConvergedMapPointSeed(c.type)) // 若深度滤波器收敛
    {
      const FloatType ref_depth = c.ref_frame->getSeedDepth(c.ref_index); // 获得收敛的深度
      ...
      if(res != Matcher::MatchResult::kSuccess)
        return false;
            ...
            isUnconvergedMapPointSeed(c.type))  // 若深度滤波器未收敛，会用当前帧更新一下深度滤波器；若成功更新，会继续流程
    {
    ...
    feature.seed_ref.keyframe = c.ref_frame; // 重叠关键帧
    feature.seed_ref.seed_id = c.ref_index;  // 在重叠关键帧中的特征点索引
  }
  else  // Map Point 情况，已有 3D 点
  {
    const PointPtr& point = c.ref_frame->landmark_vec_[c.ref_index];
    FramePtr ref_frame;
    size_t ref_feature_index;
    if(!point->getCloseViewObs(frame->pos(), ref_frame, ref_feature_index)) // 最正的视角
    {
      return false;
    }
    ...
    Reprojector::Candidate& candidate)
{
  ...
  int n_reproj = 0;
  if(ref_frame->landmark_vec_[ref_index] != nullptr) // Map Point 情况
  {
