...
void Map::getClosestNKeyframesWithOverlap(
    ...
    const size_t num_frames, // 要找多少个临近关键帧用于投影
    std::vector<FramePtr>* close_kfs) const
{
  CHECK_NOTNULL(close_kfs);
  std::vector<std::pair<FramePtr, double>> overlap_kfs;
  getOverlapKeyframes(cur_frame, &overlap_kfs); // 具有重叠视野的帧（即至少有一个特征点能落在图像平面内）
  ...
  // 提取 overlap_kfs 中 pair 的第一个元素（因为第二个元素已经在刚才计算最近距离时用过了，我们不再需要了）
