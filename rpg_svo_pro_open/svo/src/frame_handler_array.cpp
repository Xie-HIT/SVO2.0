...
UpdateResult FrameHandlerArray::processFrameBundle()
{
  ...
  std::cout << "阶段-开始处理（frame_handler_array.cpp 61）：" << print_stage(stage_) << std::endl;
...
UpdateResult FrameHandlerArray::processSecondFrame()
{
  ...
  // FIXME（xie chen）: 这段源代码会崩溃, initializer_->frames_ref_ 是空指针
  // make old frame keyframe
  if(initializer_->frames_ref_) // TODO(xie chen): ADD THIS LINE TO FIX
  {
