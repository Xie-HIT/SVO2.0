...
{
  // TODO (xie chen)：多相机配置，不是简单加 1
  svo::FramePtr first_kf = map.getKeyFrameAt(0);
  svo::FramePtr second_kf;
  for(size_t i=1; i<map.size(); ++i)
  {
    second_kf = map.getKeyFrameAt(i);
    if(second_kf->bundleId() != first_kf->bundleId())
      break;
  }
...
#ifdef SVO_LOOP_CLOSING
      if (lc_ && !last_rm_kfs.empty())
      {
        /// 回环: 位姿图优化 (多相机版本)
        for(const auto& last_rm_kf: last_rm_kfs)
        {
