      ...
      vk::Timer timer;
      {
        ...
        if (is_w_T_valid_)
        {
          ...
          backend_.transformMap(
              w_T_correction_to_apply_/* 新旧 world 坐标系原点之间的变换: w_T_new_old */,
              optimizer_options_.remove_marginalization_term_after_correction_,
              optimizer_options_.recalculate_imu_terms_after_loop); // 将后端所有关键帧的位姿和地图点进行回环校正
          for (const FramePtr& f : active_keyframes_) // 将 active_keyframes_ 的所有关键帧的位姿进行回环校正
          {
