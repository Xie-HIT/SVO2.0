    ...
    const float depth_mean)
{
  ...
  // FIXME (xie chen): ADD THIS TO FIX
  std::cout << "old: " << n_old << ",  new: " << n_new << std::endl;
  if(frame->px_vec_.cols() <= static_cast<int>(n_new+n_old))
  {
      frame->resizeFeatureStorage(n_new+n_old);
      std::cout << "扩充 frame->px_vec_ 的列数" << std::endl;
  }
  ...
  // initialize seeds
  frame->seed_mu_range_ = seed::getMeanRangeFromDepthMinMax(depth_min, depth_max); // 逆深度的最大值
  if (no_features_in_frame)
  {
...
bool updateSeed(
    ...
    Frame& ref_frame,
    const size_t& seed_index, // ref_frame_seed_index，包含新检测到的特征点
    Matcher& matcher, // 深度滤波器的 matcher（有自己的参数）
    ...
    const bool use_vogiatzis_update) // true
{
  ...
  // Create wrappers
  FeatureWrapper ref_ftr = ref_frame.getFeatureWrapper(seed_index); // 参考帧上的seed索引
  Eigen::Ref<SeedState> state = ref_frame.invmu_sigma2_a_b_vec_.col(seed_index); // seed 的状态
  ...
  Matcher::MatchResult res =
      matcher.findEpipolarMatchDirect(
        ref_frame, cur_frame, T_cur_ref, ref_ftr/* 参考帧上的seed索引 */, seed::getInvDepth(state),
