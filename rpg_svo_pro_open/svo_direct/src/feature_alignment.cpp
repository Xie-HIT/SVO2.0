...
bool align1D(
    ...
    const Eigen::Ref<GradientVector>& dir/* 沿着该方向进行极线搜索 */,   // direction in which the patch is allowed to move
...
#define SVO_DESCALE(x,n)     (((x) + (1 << ((n)-1))) >> (n))
  ...
  uint8_t __attribute__((__aligned__(16))) ref_patch[max_patch_area]; // 8位存的是0-255的像素值
  int16_t __attribute__((__aligned__(16))) ref_patch_dx[max_patch_area]; // 差分方式获得导数，即两个8位相减，用16位存储
  ...
  for(int level = max_level; level >= min_level; --level)
  {
    ...
    const Eigen::Vector2f px_ref_flt = px_ref_level_0.cast<float>() / scale - Eigen::Vector2f(halfpatch_size, halfpatch_size); // 模板点在当前金字塔层的归一化坐标
    ...
    // 以patch左上角为中心，构成一个大小为patch_size的patch
    // 给ref_patch赋值（即获得其像素值）
    for(int y=0; y<patch_size; ++y)
    {
      ...
      {
        *it_patch = *it;
        *it_dx = static_cast<int16_t>(it[1]) - it[-1]; //FIXME（xie chen）： 边缘的计算有问题
